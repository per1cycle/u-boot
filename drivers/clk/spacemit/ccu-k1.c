...
static struct spacemit_clk_table spacemit_k1_clks = {
	.clks	= {
...
ulong ccu_clk_get_rate(struct clk *clk)
{
	...
	struct clk *c = spacemit_k1_clks.clks[clk->id];
	if (!clk_valid(c))
		return 0;
...
int ccu_clk_set_parent(struct clk *clk, struct clk *parent)
{
	...
	struct clk *c = spacemit_k1_clks.clks[clk->id];
	struct clk *p = spacemit_k1_clks.clks[parent->id];
	if (!clk_valid(c))
		return 0;
	ops = ccu_clk_dev_ops(c->dev);
	if(ops->set_parent)
...
int ccu_common_init(struct clk * clk, struct spacemit_k1_clk *clk_info, struct spacemit_clk_table *clks)
{
...
static inline void ccu_clk_dm(ulong id, struct clk *clk)
{
	if (!IS_ERR(clk)){
		...
		clk->id = id;
		spacemit_k1_clks.clks[id] = clk;
	}
}

static int spacemit_k1_ccu_probe(struct udevice *dev)
{
	int ret = 0;
	struct spacemit_k1_clk *clk_info = &k1_clock_controller;
	struct spacemit_clk_table *clks = &spacemit_k1_clks;
...
static const struct udevice_id ccu_clk_ids[] = {
	{ .compatible = "spacemit,k1-ccu" },
	{ },
};

U_BOOT_DRIVER(spacemit_k1_ccu) = {
	.name = "k1-ccu",
	.id = UCLASS_CLK,
	.of_match = ccu_clk_ids,
	.ops = &ccu_clk_ops,
	.probe = spacemit_k1_ccu_probe,
};
